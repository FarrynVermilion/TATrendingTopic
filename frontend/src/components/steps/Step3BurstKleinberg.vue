    ...
    <div class="card float-up border border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
      <div class="flex items-center gap-2 mb-4">
        <svg class="w-5 h-5 text-primary" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3v11.25A2.25 2.25 0 0 0 6 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0 1 18 16.5h-2.25m-7.5 0h7.5m-7.5 0-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5m.75-9 3-3 2.148 2.148 3.1-3.1" />
        </svg>
        <h3 class="text-sm font-bold text-text uppercase tracking-wider">Burst Detection Settings</h3>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-text-muted uppercase">Top N Terms</label>
          <input v-model.number="store.analysisConfig.top_n_terms" type="number" min="5" max="200" class="w-full bg-surface-alt border border-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/20 outline-none" />
        </div>
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-text-muted uppercase">Bin Hours</label>
          <input v-model.number="store.analysisConfig.bin_hours" type="number" min="1" max="24" class="w-full bg-surface-alt border border-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/20 outline-none" />
        </div>
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-text-muted uppercase">Kleinberg Scaling (s)</label>
          <input v-model.number="store.analysisConfig.kleinberg_s" type="number" step="0.1" min="1.1" class="w-full bg-surface-alt border border-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/20 outline-none" />
        </div>
        <div class="space-y-1.5">
          <label class="text-[10px] font-bold text-text-muted uppercase">Transition (gamma)</label>
          <input v-model.number="store.analysisConfig.kleinberg_gamma" type="number" step="0.1" min="0.1" class="w-full bg-surface-alt border border-border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/20 outline-none" />
        </div>
      </div>
      <div class="mt-4 flex justify-end">
        <button class="btn-primary glow-pulse" :disabled="store.isLoading" @click="store.runBurstDetection()">
          Run Burst Detection
        </button>
      </div>
    </div>

    <div v-if="store.burstAnalysis.term_frequencies.length > 0" class="flex flex-col gap-6 float-up" style="animation-delay: 0.1s;">
      <!-- Main Header Card -->
